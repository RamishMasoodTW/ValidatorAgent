...
jobs:
  ...
  # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
  quality-gate:
    name: \u{1F6E1}\uFE0F Quality Gate & Verification (CI)
    runs-on: ubuntu-latest
    steps:
      - name: \u{1F4E5} Checkout Repository
      ...
      - name: \u{1F4E6} Clean Install Dependencies (npm ci)
        run: npm ci

      - name: \u{1F512} Dependency Security Audit
        run: npm audit --audit-level=high

      - name: \u{1F50D} Strict TypeScript Compilation
        run: npx tsc --noEmit --skipLibCheck

      - name: \u{1F9F9} Angular Lint Verification
        run: npm run lint --if-present

      - name: \u{1F9EA} Automated Headless Unit Tests
        run: npm run test:ci --if-present

      - name: \u{1F3D7}\uFE0F Angular Production Build (AOT)
  ...
  # \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
  delivery-readiness:
    name: \u{1F6A2} Deployment Readiness & Delivery (CD)
    needs: quality-gate
    runs-on: ubuntu-latest
    ...
    steps:
      ...
      - name: \u{1F4E6} Clean Install Dependencies
        run: npm ci

      - name: \u{1F3D7}\uFE0F Compile Production Bundle
        run: npm run build --if-present

      - name: \u{1F50D} Verify Compiled Distribution Artifacts & SPA Rewrite
        shell: bash
        run: |
          if [ -d "dist" ]; then
      ...
      - name: \u{1F4E4} Upload Production Distribution Artifacts
...
`;
  ...
  console.log(source_default.green("\u2714 Angular Gatekeeper Installer self-test passed successfully."));
  process.exit(0);
} else {
...
async function waitPrompt(message = "Press [Enter] to exit...") {
  ...
  return new Promise((resolve) => {
    rl.question(source_default.gray(`
${message}`), () => {
      rl.close();
      resolve();
    });
...
\u2022 Target Directory: ${targetDir}`));
  ...
  try {
    ...
    console.log(source_default.green("\u2714 Target directories created successfully."));
  } catch (err) {
    console.log(source_default.red(`\u2716 Failed to create installation directories: ${err.message}`));
    await waitPrompt();
    process.exit(1);
  }
...
`;
  ...
  const targetEnginePath = import_path.default.join(targetDir, "engine.exe");
  const candidateEnginePaths = [
    import_path.default.join(import_path.default.dirname(process.execPath), "engine.exe"),
    import_path.default.join(process.cwd(), "dist", "engine.exe"),
    import_path.default.join(process.cwd(), "engine.exe"),
    typeof __dirname !== "undefined" ? import_path.default.join(__dirname, "engine.exe") : null,
    typeof __dirname !== "undefined" ? import_path.default.join(__dirname, "..", "dist", "engine.exe") : null
  ].filter(Boolean);
  ...
  if (sourceEnginePath) {
    try {
      import_fs.default.copyFileSync(sourceEnginePath, targetEnginePath);
      console.log(source_default.green(`\u2714 Copied engine binary to: ${targetEnginePath}`));
    } catch (err) {
      console.log(source_default.yellow(`\u26A0 Could not copy engine.exe: ${err.message}`));
    }
  } else {
