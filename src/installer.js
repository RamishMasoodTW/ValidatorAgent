...
async function runInstaller() {
  ...
  if (aiProvider === 'gemini') {
    ...
    const openAiPrompt = await prompts([
      {
        type: 'password',
        name: 'key',
    ...
    console.log(chalk.green(`✔ OpenAI configured (${openAiModel}).`));
  } else if (aiProvider === 'anthropic') {
    const anthropicPrompt = await prompts([
      {
        type: 'password',
        name: 'key',
    ...
    console.log(chalk.green(`✔ Anthropic Claude configured (${anthropicModel}).`));
  } else if (aiProvider === 'deepseek') {
    const deepseekPrompt = await prompts([
      {
        type: 'password',
        name: 'key',
    ...
    console.log(chalk.green(`✔ DeepSeek configured (${deepseekModel}).`));
  } else if (aiProvider === 'groq') {
    const groqPrompt = await prompts([
      {
        type: 'password',
        name: 'key',
    ...
    console.log(chalk.green(`✔ Groq Ultra-Fast AI configured (${groqModel}).`));
  } else if (aiProvider === 'openrouter') {
    const openRouterPrompt = await prompts([
      {
        type: 'password',
        name: 'key',
    ...
    console.log(chalk.green(`✔ OpenRouter configured (${openRouterModel}).`));
